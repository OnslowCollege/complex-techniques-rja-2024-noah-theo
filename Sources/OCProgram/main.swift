...
class BlackJackApp : OCApp{
    ...
    override open func main(app: any OCAppDelegate) -> OCControl {
        ...
        let decoyHbox = OCHBox(controls: [])
        let helpHbox = OCHBox(controls: [helpButton], justifyContent: OCContentJustification.flexEnd)
        let topHbox = OCHBox(controls: [decoyHbox, helpHbox])
